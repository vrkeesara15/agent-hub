...
export default function InformaticaMigrationPage() {
  ...
  const [migrationMode, setMigrationMode] = useState<MigrationMode>('standard');
  const [dragActive, setDragActive] = useState(false);
  const fileInputRef = useRef<HTMLInputElement>(null);

  const isAdvanced = migrationMode === 'advanced';
  ...
  const getReportData = (r: InformaticaMigrationResponse) => {
    ...
    const sqlPct = r.analysis?.total_transformations ? Math.round((r.analysis.sql_convertible / r.analysis.total_transformations) * 100) : 0;
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 py-8">
      ...
      <div className="mb-6 p-4 bg-surface-card border border-surface-border rounded-card">
        <div className="flex items-center justify-between">
          <div>
            <p className="text-sm font-bold text-text-primary">Migration Mode</p>
            <p className="text-xs text-text-secondary mt-1">
              {isAdvanced
                ? 'Advanced mode processes each mapping individually with per-mapping LLM calls for better accuracy on complex workflows.'
                : 'Standard mode uses a single LLM call for fast, cost-efficient migration of simple to medium workflows.'}
            </p>
          </div>
          <div className="inline-flex bg-surface-bg border border-surface-border rounded-button overflow-hidden">
            <button
              onClick={() => { setMigrationMode('standard'); setResult(null); setAdvancedResult(null); setActiveTab('report'); }}
              className={`px-4 py-2 text-xs font-medium transition-colors ${migrationMode === 'standard' ? 'bg-brand-blue text-white' : 'text-text-secondary hover:text-text-primary'}`}
            >
              Standard
            </button>
            <button
              onClick={() => { setMigrationMode('advanced'); setResult(null); setAdvancedResult(null); setActiveTab('scorecard'); }}
              className={`px-4 py-2 text-xs font-medium transition-colors ${migrationMode === 'advanced' ? 'bg-purple-600 text-white' : 'text-text-secondary hover:text-text-primary'}`}
            >
              <span className="flex items-center gap-1.5">
                <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M13 2L3 14h9l-1 8 10-12h-9l1-8z"/></svg>
                Advanced (Complex Jobs)
              </span>
            </button>
          </div>
        </div>
        {isAdvanced && (
          <div className="mt-3 p-2.5 bg-purple-50 dark:bg-purple-950/20 border border-purple-200 dark:border-purple-800 rounded-lg">
            <p className="text-xs text-purple-700 dark:text-purple-300 flex items-center gap-1.5">
              ...
              Uses multiple LLM calls (one per mapping) for higher accuracy. Recommended for workflows with 10+ mappings or 100+ transformations.
            </p>
          </div>
        )}
      </div>
      ...
      <div className="mb-6 p-4 bg-indigo-50 dark:bg-indigo-950/30 border border-indigo-200 dark:border-indigo-800 rounded-card">
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-text-secondary mt-1">Upload your Informatica XML export to convert it into BigQuery SQL + Cloud Composer (Airflow) DAGs. Transformations are mapped to BigQuery ELT. Complex logic flags Dataflow / Dataproc. SCD patterns generate MERGE statements.</p>
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center gap-2 mb-4">
        <div className="inline-flex bg-surface-bg border border-surface-border rounded-button overflow-hidden">
          <button onClick={() => setInputMode('upload')} className={`px-4 py-2 text-xs font-medium transition-colors ${inputMode === 'upload' ? 'bg-brand-blue text-white' : 'text-text-secondary hover:text-text-primary'}`}>
            <span className="flex items-center gap-1.5">
          ...
          <button onClick={() => setInputMode('paste')} className={`px-4 py-2 text-xs font-medium transition-colors ${inputMode === 'paste' ? 'bg-brand-blue text-white' : 'text-text-secondary hover:text-text-primary'}`}>
            <span className="flex items-center gap-1.5">
      ...
      {inputMode === 'upload' && (
        <div className="mb-6">
          <input ref={fileInputRef} type="file" accept=".xml,.XML" onChange={handleFileInput} className="hidden" />
          <div onDragEnter={handleDrag} onDragLeave={handleDrag} onDragOver={handleDrag} onDrop={handleDrop} onClick={() => fileInputRef.current?.click()}
            className={`relative border-2 border-dashed rounded-card p-10 text-center cursor-pointer transition-all ${dragActive ? 'border-brand-blue bg-blue-50/50 dark:bg-blue-950/20' : fileName ? 'border-green-400 bg-green-50/30 dark:bg-green-950/10' : 'border-surface-border hover:border-brand-blue/50 hover:bg-surface-bg/50'}`}>
            {fileName ? (
              <div className="flex flex-col items-center gap-3">
                <div className="w-14 h-14 rounded-full bg-green-100 dark:bg-green-900/30 flex items-center justify-center">
                  <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="#16A34A" strokeWidth="2" strokeLinecap="round"><path d="M22 11.08V12a10 10 0 11-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg>
                </div>
                <div><p className="text-sm font-bold text-text-primary">{fileName}</p><p className="text-xs text-text-secondary mt-1">{(xmlInput.length / 1024).toFixed(1)} KB loaded</p></div>
              ...
              <div className="flex flex-col items-center gap-3">
                ...
                <div><p className="text-sm font-bold text-text-primary">Drop your Informatica XML file here</p><p className="text-xs text-text-secondary mt-1">or click to browse files</p></div>
                <span className="inline-block text-[10px] font-medium text-text-muted bg-surface-bg px-3 py-1 rounded-full border border-surface-border">Supports .xml files from PowerCenter Export</span>
              </div>
            )}
          </div>
        </div>
      ...
      {inputMode === 'paste' && (
        <div className="mb-6">
          <textarea value={xmlInput} onChange={(e) => { setXmlInput(e.target.value); setFileName(null); }} placeholder="Paste your Informatica PowerCenter XML export here..."
      ...
      <div className="flex justify-center mb-8">
        <Button variant="primary" size="lg" onClick={handleMigrate} disabled={loading || !xmlInput.trim()} className={`px-12 ${isAdvanced ? '!bg-purple-600 hover:!bg-purple-700' : ''}`}>
          {loading ? (
            <span className="flex items-center gap-2">
              <svg className="animate-spin h-4 w-4" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" fill="none"/><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"/></svg>
              {isAdvanced ? 'Advanced Analysis & Migration...' : 'Analyzing & Migrating...'}
            </span>
          ) : isAdvanced ? 'Advanced Migrate to GCP' : 'Migrate to GCP'}
        </Button>
      </div>
      ...
      {loading && (
        <div className="flex items-center justify-center py-16">
          <div className="flex flex-col items-center gap-4">
            <svg className={`animate-spin h-12 w-12 ${isAdvanced ? 'text-purple-600' : 'text-brand-blue'}`} viewBox="0 0 24 24">
        ...
        <div className="space-y-6">
          ...
          <div className="flex items-center justify-between flex-wrap gap-3">
            <h2 className="text-lg font-bold text-text-primary flex items-center gap-2">
              <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke={isAdvanced ? '#9333EA' : '#6366F1'} strokeWidth="2" strokeLinecap="round"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/><polyline points="14 2 14 8 20 8"/></svg>
              Migration Results
              {isAdvanced && <span className="text-xs font-medium px-2 py-0.5 rounded-full bg-purple-100 text-purple-700 dark:bg-purple-900 dark:text-purple-300">Advanced</span>}
              {fileName && <span className="text-sm font-normal text-text-secondary ml-2">({fileName})</span>}
            </h2>
            <div className="flex items-center gap-3">
              {advancedResult?.scorecard && (
                <div className={`flex items-center gap-2 px-4 py-2 rounded-full border text-sm font-bold ${scoreBgColor(advancedResult.scorecard.overall_score)}`}>
                  <span className={scoreColor(advancedResult.scorecard.overall_score)}>Score: {advancedResult.scorecard.overall_score}%</span>
                </div>
              )}
              <button onClick={handleDownloadAll} className="flex items-center gap-2 px-4 py-2 rounded-full border border-brand-blue text-brand-blue text-sm font-bold hover:bg-brand-blue hover:text-white transition-colors">
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
            ...
            <div className="p-4 bg-surface-card border border-surface-border rounded-card text-center">
              <div className="text-2xl font-bold text-indigo-600">{advancedResult?.mapping_results?.length || result.transformation_map?.length || 0}</div>
              <div className="text-xs text-text-secondary mt-1">{advancedResult ? 'Mappings Processed' : 'Mappings Created'}</div>
            </div>
          </div>
          ...
          <div className="flex gap-1 border-b border-surface-border overflow-x-auto">
            {([
              ...(advancedResult ? [
                { key: 'scorecard' as ActiveTab, label: 'Scorecard', icon: <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M22 12h-4l-3 9L9 3l-3 9H2"/></svg> },
              ...
              { key: 'dag' as ActiveTab, label: 'Airflow DAG', icon: <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="3"/></svg> },
              { key: 'map' as ActiveTab, label: 'Transformation Map', icon: <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="3" y="3" width="7" height="7"/><rect x="14" y="14" width="7" height="7"/></svg> },
          ...
          {/* ══════ Tab: Parameters ══════ */}
          {activeTab === 'params' && advancedResult && (
            <div className="bg-surface-card border border-surface-border rounded-card overflow-hidden">
              <div className="px-4 py-3 border-b border-surface-border bg-surface-bg/50">
                <span className="text-xs font-medium text-text-secondary uppercase tracking-wide">Extracted Parameters ($$variables)</span>
              </div>
              {(!advancedResult.parameters || advancedResult.parameters.length === 0) ? (
                <div className="p-8 text-center text-sm text-text-muted">No $$parameters found in this workflow.</div>
              ) : (
                <div className="p-4 overflow-x-auto">
                  <table className="w-full text-sm">
                    <thead><tr className="border-b border-surface-border">
                      <th className="text-left py-2.5 px-3 text-text-secondary font-medium">Parameter</th>
                      <th className="text-left py-2.5 px-3 text-text-secondary font-medium">BigQuery Equivalent</th>
                      <th className="text-left py-2.5 px-3 text-text-secondary font-medium">Guessed Type</th>
                      <th className="text-left py-2.5 px-3 text-text-secondary font-medium">Default</th>
                    </tr></thead>
                    <tbody>
                      {advancedResult.parameters.map((p, i) => (
                        <tr key={i} className="border-b border-surface-border/50 last:border-0 hover:bg-surface-bg/30 transition-colors">
                          <td className="py-2.5 px-3 font-mono text-purple-700 dark:text-purple-300 text-xs">$${p.name}</td>
                          <td className="py-2.5 px-3 font-mono text-blue-700 dark:text-blue-300 text-xs">@{p.name}</td>
                          <td className="py-2.5 px-3"><span className="inline-block px-2 py-0.5 rounded-full text-[10px] font-medium bg-gray-100 text-gray-600 dark:bg-gray-800 dark:text-gray-400">{p.type_guess}</span></td>
                          <td className="py-2.5 px-3 text-xs text-text-muted">{p.default_value || '—'}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div className="bg-gradient-to-br from-green-50 to-emerald-50/50 dark:from-green-950/20 dark:to-emerald-950/10 border border-green-200 dark:border-green-800 rounded-card overflow-hidden">
                  <div className="px-5 py-4 border-b border-green-200 dark:border-green-800 bg-green-100/50 dark:bg-green-900/20">
                    <h3 className="text-sm font-bold text-green-800 dark:text-green-300 flex items-center gap-2">
                      <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#16A34A" strokeWidth="2.5" strokeLinecap="round"><path d="M22 11.08V12a10 10 0 11-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg>
                      What Went Well ({good.length})
                    </h3>
                  </div>
                  <div className="p-5 space-y-4">
                    {good.length === 0 ? <p className="text-sm text-text-muted">No highlights for this migration.</p> : good.map((item, i) => (
                      <div key={i} className="flex items-start gap-3">
                        <div className="mt-0.5 flex-shrink-0 w-8 h-8 rounded-full bg-green-100 dark:bg-green-900/40 flex items-center justify-center"><GoodIcon type={item.icon} /></div>
                        <div className="min-w-0"><p className="text-sm font-semibold text-green-800 dark:text-green-300">{item.label}</p><p className="text-xs text-green-700/80 dark:text-green-400/70 mt-0.5 leading-relaxed">{item.detail}</p></div>
                      </div>
                    ))}
                  </div>
                </div>
                <div className="bg-gradient-to-br from-amber-50 to-orange-50/50 dark:from-amber-950/20 dark:to-orange-950/10 border border-amber-200 dark:border-amber-800 rounded-card overflow-hidden">
                  ...
                  <div className="p-5 space-y-4">
                    {attention.length === 0 ? (
                      ...
                      <div key={i} className="flex items-start gap-3">
                        <div className={`mt-0.5 flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center ${item.severity === 'critical' ? 'bg-red-100 dark:bg-red-900/40' : 'bg-amber-100 dark:bg-amber-900/40'}`}>
                          {item.severity === 'critical' ? <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#DC2626" strokeWidth="2"><circle cx="12" cy="12" r="10"/><line x1="15" y1="9" x2="9" y2="15"/><line x1="9" y1="9" x2="15" y2="15"/></svg> : <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#D97706" strokeWidth="2"><path d="M10.29 3.86L1.82 18a2 2 0 001.71 3h16.94a2 2 0 001.71-3L13.71 3.86a2 2 0 00-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg>}
                        </div>
                        <div className="min-w-0">
                          <p className={`text-sm font-semibold ${item.severity === 'critical' ? 'text-red-700 dark:text-red-400' : 'text-amber-800 dark:text-amber-300'}`}>{item.label}<span className={`ml-2 inline-block text-[10px] font-bold uppercase px-1.5 py-0.5 rounded-full ${item.severity === 'critical' ? 'bg-red-200/60 text-red-700 dark:bg-red-800/40 dark:text-red-300' : 'bg-amber-200/60 text-amber-700 dark:bg-amber-800/40 dark:text-amber-300'}`}>{item.severity}</span></p>
                ...
                {result.recommendations && result.recommendations.length > 0 && (
                  <div className="lg:col-span-2 bg-gradient-to-br from-blue-50 to-indigo-50/50 dark:from-blue-950/20 dark:to-indigo-950/10 border border-blue-200 dark:border-blue-800 rounded-card overflow-hidden">
                    ...
                    <div className="p-5"><div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                ...
                <div className="lg:col-span-2 p-5 bg-surface-card border border-surface-border rounded-card">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <span className="text-sm font-bold text-brand-blue">{result.analysis?.total_transformations ? Math.round((result.analysis.sql_convertible / result.analysis.total_transformations) * 100) : 0}%</span>
                  </div>
                  <div className="w-full h-3 bg-gray-200 dark:bg-gray-700 rounded-full overflow-hidden">
                    <div className="h-full rounded-full transition-all duration-700 bg-gradient-to-r from-green-500 to-emerald-400" style={{ width: `${result.analysis?.total_transformations ? Math.round((result.analysis.sql_convertible / result.analysis.total_transformations) * 100) : 0}%` }} />
                  </div>
                ...
                <div className="lg:col-span-2 p-5 bg-gradient-to-r from-indigo-50 to-blue-50 dark:from-indigo-950/20 dark:to-blue-950/20 border border-indigo-200 dark:border-indigo-800 rounded-card">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-xs text-indigo-600/70 dark:text-indigo-400/60 mt-1">Download all migration artifacts as a single ZIP package</p>
                    </div>
                    <button onClick={handleDownloadAll} className="flex items-center gap-2 px-5 py-2.5 rounded-button bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-medium transition-colors">
                      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M21 15v4a2 2 0 01-2 2H5a2 2 0 01-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>
                      Download All (.zip)
                    </button>
                  </div>
                  <div className="flex flex-wrap gap-2 mt-4">
                    {[
                      ...
                      <button key={item.label} onClick={item.action} className="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium bg-white/70 dark:bg-black/20 border border-indigo-200 dark:border-indigo-700 text-indigo-700 dark:text-indigo-300 hover:bg-white dark:hover:bg-black/40 transition-colors">
          ...
          {activeTab === 'sql' && (
            <div className="bg-surface-card border border-surface-border rounded-card overflow-hidden">
              <div className="flex items-center justify-between px-4 py-3 border-b border-surface-border bg-surface-bg/50">
                ...
                <div className="flex items-center gap-3">
                  <button onClick={() => downloadFile(result.bigquery_sql, `${getBaseFilename()}_bigquery.sql`)} className="text-xs text-brand-blue hover:underline flex items-center gap-1"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M21 15v4a2 2 0 01-2 2H5a2 2 0 01-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>Download .sql</button>
                  <button onClick={() => navigator.clipboard.writeText(result.bigquery_sql)} className="text-xs text-brand-blue hover:underline flex items-center gap-1"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="9" y="9" width="13" height="13" rx="2" ry="2"/><path d="M5 15H4a2 2 0 01-2-2V4a2 2 0 012-2h9a2 2 0 012 2v1"/></svg>Copy</button>
                </div>
              </div>
              <pre className="p-4 text-sm font-mono text-text-primary overflow-x-auto max-h-[500px] overflow-y-auto whitespace-pre">{result.bigquery_sql}</pre>
            </div>
          )}
          ...
          {activeTab === 'dag' && (
            <div className="bg-surface-card border border-surface-border rounded-card overflow-hidden">
              <div className="flex items-center justify-between px-4 py-3 border-b border-surface-border bg-surface-bg/50">
                ...
                <div className="flex items-center gap-3">
                  <button onClick={() => downloadFile(result.airflow_dag, `${getBaseFilename()}_airflow_dag.py`)} className="text-xs text-brand-blue hover:underline flex items-center gap-1"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M21 15v4a2 2 0 01-2 2H5a2 2 0 01-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>Download .py</button>
                  <button onClick={() => navigator.clipboard.writeText(result.airflow_dag)} className="text-xs text-brand-blue hover:underline flex items-center gap-1"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="9" y="9" width="13" height="13" rx="2" ry="2"/><path d="M5 15H4a2 2 0 01-2-2V4a2 2 0 012-2h9a2 2 0 012 2v1"/></svg>Copy</button>
                </div>
              </div>
              <pre className="p-4 text-sm font-mono text-text-primary overflow-x-auto max-h-[500px] overflow-y-auto whitespace-pre">{result.airflow_dag}</pre>
            </div>
          )}
          ...
          {activeTab === 'scd' && result.scd_merge && (
            <div className="bg-surface-card border border-surface-border rounded-card overflow-hidden">
              <div className="flex items-center justify-between px-4 py-3 border-b border-surface-border bg-surface-bg/50">
                ...
                <div className="flex items-center gap-3">
                  <button onClick={() => downloadFile(result.scd_merge, `${getBaseFilename()}_scd_merge.sql`)} className="text-xs text-brand-blue hover:underline flex items-center gap-1"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M21 15v4a2 2 0 01-2 2H5a2 2 0 01-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>Download .sql</button>
                  <button onClick={() => navigator.clipboard.writeText(result.scd_merge)} className="text-xs text-brand-blue hover:underline flex items-center gap-1"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="9" y="9" width="13" height="13" rx="2" ry="2"/><path d="M5 15H4a2 2 0 01-2-2V4a2 2 0 012-2h9a2 2 0 012 2v1"/></svg>Copy</button>
                </div>
              </div>
              <pre className="p-4 text-sm font-mono text-text-primary overflow-x-auto max-h-[500px] overflow-y-auto whitespace-pre">{result.scd_merge}</pre>
            </div>
          )}
          ...
          {activeTab === 'map' && (
            <div className="bg-surface-card border border-surface-border rounded-card overflow-hidden">
              <div className="flex items-center justify-between px-4 py-3 border-b border-surface-border bg-surface-bg/50">
                ...
                <button onClick={() => downloadFile(buildTransformMapCSV(result), `${getBaseFilename()}_transformation_map.csv`)} className="text-xs text-brand-blue hover:underline flex items-center gap-1"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M21 15v4a2 2 0 01-2 2H5a2 2 0 01-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>Download .csv</button>
              </div>
              <div className="p-4 overflow-x-auto">
                <table className="w-full text-sm">
                  <thead><tr className="border-b border-surface-border">
                  ...
                  <tbody>
                    {result.transformation_map?.map((item, i) => (
                      <tr key={i} className="border-b border-surface-border/50 last:border-0 hover:bg-surface-bg/30 transition-colors">
                        ...
                        <td className="py-2.5 px-3"><span className={`inline-block px-2.5 py-0.5 rounded-full text-xs font-medium ${item.type === 'sql' ? 'bg-green-100 text-green-700 dark:bg-green-900 dark:text-green-300' : item.type === 'dataflow' ? 'bg-amber-100 text-amber-700 dark:bg-amber-900 dark:text-amber-300' : 'bg-blue-100 text-blue-700 dark:bg-blue-900 dark:text-blue-300'}`}>{item.type}</span></td>
                      </tr>
                    ))}
                  </tbody>
                </table>
